import json
import os

_HERE = os.path.dirname(os.path.abspath(__file__))

# 优先使用 debug 配置，不存在则回退到默认配置
_config_debug = os.path.join(_HERE, 'config_debug.json')
_config_default = os.path.join(_HERE, 'config.json')
CONFIG_FILE = _config_debug if os.path.exists(_config_debug) else _config_default
"""当前生效的静态配置文件路径。可通过 set_config_file() 在运行时修改。"""


def set_config_file(path: str):
    """设置自定义配置文件路径，覆盖默认的 config.json。

    Args:
        path: 自定义配置文件的路径（绝对或相对路径）。

    应在程序入口（main.py）解析命令行参数后、首次调用 get_config() 之前调用。
    """
    global CONFIG_FILE
    CONFIG_FILE = os.path.abspath(path)
    if not os.path.exists(CONFIG_FILE):
        raise FileNotFoundError(f'Configuration file does not exist: {CONFIG_FILE}')


# ── 静态配置（config.json）────────────────────────────────────────────────────

def get_config() -> dict:
    """读取静态配置与运行时配置，合并后返回统一字典。"""
    with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
        config = json.load(f)
    # 运行时字段覆盖静态字段（如有同名）
    config.update(_working_config)

    # 确保所有运行时字段都有默认值
    defaults = {
        'wait': 10,
        'wait_download': 60,
        'read_max_lines': 1000,
        'read_max_size_kb': 100,
        'where': config.get('work_dir') or os.getcwd(),
    }
    for key, default_value in defaults.items():
        if key not in config:
            config[key] = default_value

    # work_dir 未指定、为空字符串、为 null、或不存在时，以运行的目录为默认值
    if not config.get('work_dir') or not os.path.exists(config['work_dir']):
        config['work_dir'] = os.getcwd()
    # 向后兼容：where 为空时回退到 work_dir
    if not config.get('where'):
        config['where'] = config['work_dir']
    return config


def get_static_config() -> dict:
    """仅读取静态配置 (config.json)，不合并运行时配置。"""
    with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
        return json.load(f)


# ── 运行时配置（模块级变量，内存维护，不持久化）───────────────────────────

_working_config: dict = {}


def get_working_config() -> dict:
    """获取运行时配置字典。"""
    return _working_config


def set_where(path: str):
    """更新当前工作目录。"""
    _working_config['where'] = path


def set_wait(seconds: int):
    """更新通用超时时长。"""
    _working_config['wait'] = seconds


def set_wait_download(seconds: int):
    """更新浏览器下载专用超时时长。"""
    _working_config['wait_download'] = seconds


def set_read_limits(max_lines: int | None = None, max_size_kb: int | None = None):
    """更新文件阅读限制。

    Args:
        max_lines:    最大行数上限，不超过 50000 行。
        max_size_kb:  最大体积上限（KB），不超过 5120 KB（5 MB）。
    """
    if max_lines is not None:
        _working_config['read_max_lines'] = min(max_lines, 50000)
    if max_size_kb is not None:
        _working_config['read_max_size_kb'] = min(max_size_kb, 5120)


# ── 初始化运行时状态 ─────────────────────────────────────────────────────────
# 应在程序入口（main.py）显式调用，而非导入时自动执行

def initialize_working_config():
    """初始化运行时配置，每次启动从默认工作目录开始。

    应在程序入口（main.py）启动时显式调用一次。
    """
    with open(CONFIG_FILE, encoding='utf-8') as f:
        _static = json.load(f)
    work_dir = _static.get('work_dir') or os.getcwd()
    if not os.path.exists(work_dir):
        work_dir = os.getcwd()
    _working_config['where'] = work_dir
    _working_config['wait'] = 10
    _working_config['wait_download'] = 60
    _working_config['read_max_lines'] = 1000
    _working_config['read_max_size_kb'] = 100